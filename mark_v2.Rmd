...
O `dplyr` também possui uma função extremamente utilizada no R (que inclusive foi utilizada o exemplo a cima), o pipe (`%>%`). Ele é usado para expressar claramente uma sequência de múltiplas operações. Mas porque devo usar o pipe? Bom, veja algumas vantagens: 
...
O forcats é um pacote com diversas funções com o objetivo de resolver problemas com fatores. Por exemplo, esse pacote pode ser usado para alteração da ordem dos níveis ou dos valores dos fatores. Vamos ver algumas de suas principais funções: 
...
- Torne os valores omissos implícitos explícitos com complete(); tornar explícitos os valores ausentes implícitos com drop_na(); substitua os valores ausentes pelo valor seguinte / anterior por fill(), ou um valor 

Agora vamos utilizar a função `drop_na` citada acima  para remover as  linhas com valores vazios e selecionar apenas as variáveis que iremos utilizar. 
